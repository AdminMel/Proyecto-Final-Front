...
/* ===========================
   PATCH: Avoid ApexCharts "Element not found"
   Works even with minified template scripts
   =========================== */
function chart () {
  try {
    if (!window.ApexCharts || !window.ApexCharts.prototype) return;

    var _render = window.ApexCharts.prototype.render;

    window.ApexCharts.prototype.render = function () {
      try {
        var el = this && this.el ? this.el : null;

        // If element doesn't exist in DOM, silently skip render
        if (!el || !document.body.contains(el)) {
          return Promise.resolve();
        }
      } catch (e) {
        // If anything goes wrong, do not crash the app
        return Promise.resolve();
      }
